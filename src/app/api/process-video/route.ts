    ...
    (async () => {
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          exec(
            `ffmpeg -i ${videoPath} -vn -ab 64k -ac 1 -ar 16000 ${audioPath}`,
        ...
        await new Promise<void>((resolve, reject) => {
          exec(
            `ffmpeg -i ${audioPath} -f segment -segment_time 600 -c copy ${segmentDir}/output%03d.mp3`,
            (error: Error | null, stdout: string, stderr: string) => {
              if (error) {
        ...
        for (let i = 0; i < segmentFiles.length; i++) {
          ...
          await sendProgress(
            `음성 인식 진행 중... (${i + 1}/${segmentFiles.length})`
          );
          ...
          const segmentDuration = transcription.segments.reduce(
